...
router.get('/', (req, res) => {
  ...
  db.query(query, params, (err, result) => {
    ...
    // Fetch boites_details for each archive
    let processed = 0;
...
router.post('/', (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      // Insert boites details if provided
...
router.put('/:id', (req, res) => {
  ...
  db.query('SELECT id, ref FROM archives WHERE id = ?', [archiveId], (err, results) => {
    ...
    // Update archive - ensure we're only updating by ID to avoid unique constraint violations
    db.query(
      `UPDATE archives 
      ...
      (err, result) => {
        ...
        // Delete old boites_details and insert new ones
        if (boites_details && boites_details.length > 0) {
        ...
        db.query(
          ...
          (notifErr) => {
            if (notifErr) {
              ...
              // Still send success response - the archive status was updated
            }
